...
prefix <- "/mnt/dv/wid/projects7/Roy-Maize-multi-omics/Otegui_Maize_RNAseq/results/merlin_merged_N_C_zones/output/processed/"
all_nodes_file <- paste(prefix, "MERLIN-VIZ/net0.6_nodes.txt", sep = "")
edge_list_file <- paste(prefix, "nets/net_conf_common_names_0.6.txt", sep = "")
module2gene_file <- paste(prefix, "modules_renumbered_common/map_out_0.3_module_geneset_remapped2_avg_common_names_reordered.txt", sep = "")
module_file <- paste(prefix, "modules_renumbered_common/genesets/module.0.3_common.txt", sep = "")
go_file <- paste(prefix, "MERLIN-VIZ/go_file.txt", sep = "")
regulator_enrich_file <- paste(prefix, "modules_renumbered_common/enrichments/merlin.0.6.0.3_common_details.txt", sep = "")
go_enrich_file <- paste(prefix, "modules_renumbered_common/enrichments/go.0.3_common_details.txt", sep = "")
gene2genename_file <- NA
gene_desc_file <- NA
regulator_list_file <- paste(prefix, "regulators_updateTF_common.txt", sep = "")
#regulator_list_file <- NA
expression_file <- paste(prefix, "modules_renumbered_common/enrichments/reordered_zeromean_tp_common_names_format.txt", sep = "")
